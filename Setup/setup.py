...
dic={ "activebackground":"#d9d9d9", "activeforeground":"#000000", "background":"#f0f0f0", "borderwidth":"0", "disabledforeground":"#a3a3a3", "font":font12, "foreground":"#000000", "highlightbackground":"#d9d9d9", "highlightcolor":"black", "pady":"0",}
dic2=dic.copy()
...
def alet(str_,lis={'M':'%','3√':'cube','√':'root','²':'**2','x':'*'}):
    for i in lis:
...
class Kalkulyator:
    def __init__(self, root):
        ...
        self.e = Entry(self.root, background="#e6e6e6", borderwidth="2",disabledforeground="#a3a3a3", font=font10, foreground="#000000",
                       insertbackground="black", justify=RIGHT,
                       width=14,textvariable=self.main_var)
        ...
        Button(self.root, text='''Sil''', width=63,command=self.sil,**dic)
        ...
        Button(self.root, text='''+-''', width=63,command=lambda:self.rus(),**dic)
        Button(self.root, text=''')''', width=63,command=lambda:self.btn_text(")"),**dic)
        Button(self.root, text='''=''', width=63,command=lambda:self.error(),**dic)
        Button(self.root, text='''.''', width=63,command=lambda:self.btn_text("."),**dic)
        Button(self.root,text='''/''', width=63,command=lambda:self.btn_text("/"),**dic)
        Button(self.root, text='''+''', width=63,command=lambda:self.btn_text("+"),**dic)
    ...
    def error(self):
        if self.main_var.get()!='':
            try:
                self.main_var.set(eval(alet(self.main_var.get())))
            except:
                self.main_var.set(" '0'-a bölmə yoxdur")
